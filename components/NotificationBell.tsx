        ...
        <div className="fixed top-6 right-6 z-[100]">
            <div className="relative">
                <motion.button
                    ...
                    className="w-11 h-11 bg-[var(--bg-card)]/90 backdrop-blur-md border border-[var(--border-light)] shadow-md rounded-full flex items-center justify-center relative hover:bg-[var(--bg-card)] transition-colors btn-interactive"
                >
                    <Bell size={20} className="text-[var(--text-primary)]" />
                    {unreadCount > 0 && (
                        <span className="absolute top-0 right-0 w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white shadow-sm animate-pulse">
                ...
                {isOpen && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="absolute top-14 right-0 w-80 bg-[var(--bg-card)] rounded-3xl shadow-xl border border-[var(--border-light)] overflow-hidden origin-top-right"
                        >
                                    ...
                                    <div className="divide-y divide-gray-50">
                                        {notifications.map(notif => (
                                            <div
                                                ...
                                                className={`p-4 hover:bg-[var(--bg-base)] transition-colors cursor-pointer ${!notif.isRead ? 'bg-primary/5' : ''}`}
                                            >
                                                <div className="flex gap-3">
                                                    <span className="text-base flex-shrink-0 mt-0.5">{getNotificationIcon(notif.type)}</span>
                                                    <div className="flex-1 min-w-0">
                                                        <p className={`text-sm leading-snug ${!notif.isRead ? 'font-bold text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}`}>
                                                            {notif.content}
                                                        </p>
