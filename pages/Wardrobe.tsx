...
const Wardrobe: React.FC<WardrobeProps> = ({ garments, onAddGarment, onRemoveGarment, onUpdateGarment }) => {
  ...
  // State for Sell Flow
  const [isSelling, setIsSelling] = useState(false); // Controls the "Select from Wardrobe" modal
  const [selectedForSale, setSelectedForSale] = useState<Garment | null>(null); // The garment chosen to sell
  const [salePrice, setSalePrice] = useState(''); // Price input
  ...
  const confirmSale = () => {
      if (selectedForSale && salePrice) {
          ...
          setSelectedForSale(null);
          setSalePrice('');
          setActiveView('sales'); // Auto switch to sales tab
      }
  };
  ...
  const openDetailModal = (item: Garment) => {
      setDetailItem({
          id: item.id,
          title: item.type, // Map garment type to title for demo
          price: item.price || 0,
          image: item.imageUrl,
          user: "Yo",
          avatar: "https://ui-avatars.com/api/?name=Sofia&background=0F4C5C&color=fff",
          description: "Esta prenda es de mi armario personal. ¡Espero que te guste!",
          isOwnItem: true // Special flag for own items
      });
  };
  ...
  return (
    <div className="h-full flex flex-col relative bg-gray-50">
      ...
      {/* Top Section: Header & Main Selector */}
              ...
              <div className="bg-gray-100 p-1.5 rounded-2xl flex relative font-medium text-sm">
                {(['closet', 'suitcase', 'sales'] as ViewType[]).map((view) => (
                    <button
                        ...
                        className={`flex-1 py-2 rounded-xl transition-all duration-300 text-xs font-bold uppercase tracking-wide flex items-center justify-center space-x-1 ${
                            activeView === view 
                 ...
                 <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                     <div>
                         ...
                         <p className="text-xs text-gray-500">
                             {selectedForSale ? 'Define el precio de venta' : 'Selecciona de tu armario'}
                         </p>
                     </div>
                     ...
                     {!selectedForSale ? (
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            {garments.filter(g => !g.forSale).length === 0 && (
                                <p className="col-span-2 text-center text-gray-500 mt-10">No tienes prendas disponibles para vender.</p>
                            )}
                        </div>
                     ) : (
                         <div className="flex flex-col items-center">
                             ...
                             <button 
                                ...
                                className="text-sm text-gray-500 mb-6 underline"
                             >
                                 Elegir otra prenda
                             </button>
                             ...
                             <button 
                                disabled={!salePrice}
                                onClick={confirmSale}
                                className="w-full bg-emerald-600 disabled:bg-gray-300 text-white font-bold py-4 rounded-2xl shadow-lg shadow-emerald-200 transition-colors"
                             >
                                 Publicar Venta
                             </button>
                         </div>
