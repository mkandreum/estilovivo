...
const Suitcase: React.FC<SuitcaseProps> = ({ trips, garments, onAddTrip, onDeleteTrip, onUpdateTrip, isEmbedded = false }) => {
    ...
    useEffect(() => {
        const handleTripIdUpdated = (e: Event) => {
            const detail = (e as CustomEvent).detail;
            if (detail?.oldId && detail?.newId) {
                setActiveTripId(prev => (prev === detail.oldId ? detail.newId : prev));
            }
        };
        window.addEventListener('ev:trip-id-updated', handleTripIdUpdated);
        return () => window.removeEventListener('ev:trip-id-updated', handleTripIdUpdated);
    }, []);
