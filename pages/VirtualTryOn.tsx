...
function autoPos(pose: BodyPose | null, type: string, pw: number, ph: number): { x: number; y: number; w: number; h: number } {
  ...
  const wBase = pw > 0 ? pw : 400
  const hBase = ph > 0 ? ph : 600
  if (pose && pw > 0 && ph > 0) {
    const s = smartAutoPlace(pose, type, wBase, hBase)
    return { x: s.x, y: s.y, w: s.width, h: s.height }
  }
  const r = { x: 0, y: 0, w: 0, h: 0 }
  if (/dress|vestido|enterizo/.test(t)) { r.w = wBase * 0.55; r.h = hBase * 0.48; r.x = (wBase - r.w) / 2; r.y = hBase * 0.14 }
  else if (/bottom|pantal|falda|short|jean|trouser/.test(t)) { r.w = wBase * 0.42; r.h = hBase * 0.28; r.x = (wBase - r.w) / 2; r.y = hBase * 0.44 }
  else if (/outer|chaqueta|abrigo|saco|jacket|coat/.test(t)) { r.w = wBase * 0.6; r.h = hBase * 0.36; r.x = (wBase - r.w) / 2; r.y = hBase * 0.12 }
  else if (/shoe|zapat|bota|sandal|boot/.test(t)) { r.w = wBase * 0.2; r.h = hBase * 0.08; r.x = (wBase - r.w) / 2; r.y = hBase * 0.9 }
  else if (/accesorio|sombrero|gorra|bolso|gafas|collar/.test(t)) { r.w = wBase * 0.25; r.h = hBase * 0.15; r.x = (wBase - r.w) / 2; r.y = hBase * 0.01 }
  else { r.w = wBase * 0.48; r.h = hBase * 0.3; r.x = (wBase - r.w) / 2; r.y = hBase * 0.14 }
  return r
}
