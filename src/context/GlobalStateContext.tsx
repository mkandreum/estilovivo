...
export const GlobalStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const saveLook = useCallback(async (look: Look, onAfterSave?: () => void) => {
        let isEdit = false;
        setLooks(prev => {
            isEdit = prev.some(l => l.id === look.id);
            return prev;
        });

        const tempId = isEdit ? look.id : (look.id || `temp-${Date.now()}`);
        const optimisticLook = { ...look, id: tempId };

        setLooks(prev => {
            const alreadyExists = prev.some(l => l.id === tempId);
    ...
    const addTrip = useCallback(async (newTrip: Trip) => {
        ...
        try {
            ...
            if (saved.id && saved.id !== newTrip.id) {
                window.dispatchEvent(new CustomEvent('ev:trip-id-updated', { detail: { oldId: newTrip.id, newId: saved.id } }));
            }
        } catch (error) {
